    ...
    <div className="h-full overflow-y-auto relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        <img
          ...
          alt="arrow icon"
          className=" min-md:hidden max-w-5"
          onClick={() => {
        ...
        <p className="text-white flex-1 text-lg flex items-center gap-2">
          Martin<span className="w-2 h-2 rounded-full bg-green-500"></span>
        </p>
      ...
      {/* Chat Messages */}
      <div className="flex flex-col items-between h-[calc(100%-120px)] overflow-y-auto scroll-container p-3 pb-6">
        {messagesDummyData.map((message, index) => (
          ...
          >
            {message.image ? (
              <img
                src={message.image}
                className="max-w-[230] border border-r-gray-700 rounded-lg overflow-hidden mb-8"
              />
            ) : (
              <p
                className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-violet-500/30 text-white ${
                  message.senderId === "680f50e4f10f3cd28382ecf9" ? "rounded-br-none" : "rounded-bl-none"
                }`}
              >
                {message.text}
              </p>
            ...
            <div className="text-center text-xs">
              <img src={message.senderId === "680f50e4f10f3cd28382ecf9" ? assets.avatar_icon : assets.profile_martin} alt=""  className="rounded-full w-7"/>
